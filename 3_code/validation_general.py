...
if __name__ == '__main__':
    ...
    # Specifying the model type that will be evaluated
    model_type = "TCN"

    # Loading in the hyperparameters
    hyperparams_loc = "../2_trained_models/TCN/trained_models/i7/it_1_norm/hyperparams_TCN_it_1.csv"
    hyperparams_df = pd.read_csv(hyperparams_loc, delimiter=";")

    # Reading in the num of models and model locations
    model_folder_loc = "../2_trained_models/TCN/trained_models/i7/it_1_norm/state_models/lon/"
    ...
    csv_save_loc = "../2_trained_models/TCN/trained_models/i7/it_1_norm/results/lon/"
    ...
    for i in range(len(validation_files)):
        ...
        for j in range(num_of_models):
            ...
            # Assigning the parameters for the models

            if model_type == "TCN":
                seq_len = int(checkpoint.get("sequence_length"))
                input_size = int(checkpoint.get("fixed_input_size"))
                channels_per_layer = list(checkpoint.get("channels_per_layer"))
                dilation_schedule = list(checkpoint.get("dilation_schedule"))
                convolution_per_block = int(checkpoint.get("convolution_per_block"))
                kernel_size = int(checkpoint.get("kernel_size"))
                dropout = int(checkpoint.get("fixed_dropout"))

                from classes import SpeedEstimatorTCN

                model = SpeedEstimatorTCN(input_size, hidden_size, num_layers, output_size).to(device)
                ...
                from classes import SpeedEstimatorRNN  # local import to avoid changing earlier imports
            ...
            from classes import SpeedEstimatorTCN  # local import to avoid changing earlier imports
            model = SpeedEstimatorTCN(input_size, hidden_size, num_layers, output_size).to(device)
            model.load_state_dict(checkpoint['model_state_dict'])
            model.eval()
